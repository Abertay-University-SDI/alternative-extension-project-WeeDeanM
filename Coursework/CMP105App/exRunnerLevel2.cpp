...
int getRandomInt3(int min, int max)
{
...
exRunnerLevel2::exRunnerLevel2(sf::RenderWindow* hwnd, Input* in, GameState* gs, AudioManager* aud, TextureManager* tm)
{
	window = hwnd;
	input = in;
	gameState = gs;
	...
	while (placementIndex < distance)
	{
		...
		placementIndex += getRandomInt3(650, 800);
		// harder in the back half.
		if (placementIndex > distance / 2) placementIndex -= getRandomInt3(100, 250);
...
void exRunnerLevel2::handleInput(float dt)
{
...
bool exRunnerLevel2::colliding(GameObject obj)
{
...
void exRunnerLevel2::update(float dt)
{
	// race over;
